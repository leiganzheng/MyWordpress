			...
			<?php while ( $r->have_posts() ) :
					...
					if ( has_post_thumbnail() ) {
						if ( function_exists( 'aq_resize' ) ) {
							global $mf_width, $mf_height, $mf_crop;
							
							$mf_width = ! empty( $mf_width ) ? $mf_width : '180';
							$mf_height = ! empty( $mf_height ) ? $mf_height : '135';
							$mf_crop = isset( $mf_crop ) ? true : false;
							
							$img = wp_get_attachment_image_src( get_post_thumbnail_id( $GLOBALS['post']->ID ), 'full' );
							$thumbnail = $img[0];						
							$thumbnail = aq_resize( $thumbnail, $mf_width, $mf_height, $mf_crop, true, true );
						}
						else {
