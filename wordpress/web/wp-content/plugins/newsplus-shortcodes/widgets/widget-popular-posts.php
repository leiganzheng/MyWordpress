 ...
 * Display most commented posts from specified categories.
 */
...
class NewsPlus_Popular_Posts extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Popular Posts', 'newsplus' ) : $instance['title'] );
		...
		if ( $r->have_posts() ) :
			...
			<?php while ( $r->have_posts() ) :
					...
					$post_meta = apply_filters( 'widget_recent_posts_meta', sprintf( '<span class="entry-meta"><a href="%1$s" title="%2$s" class="post-time"><time class="entry-date" datetime="%3$s">%4$s</time></a>%5$s</span>',
					...
					if ( has_post_thumbnail() && false == $hide_thumb ) {
						if ( function_exists( 'aq_resize' ) ) {
							global $list_small_width, $list_small_height, $list_small_crop;
							$img = '';
							
							$list_small_width = ! empty( $list_small_width ) ? $list_small_width : '128';
							$list_small_height = ! empty( $list_small_height ) ? $list_small_height : '96';
							$list_small_crop = ! empty( $list_small_crop ) ? true : false;
							
							$img_src = wp_get_attachment_image_src( get_post_thumbnail_id( $GLOBALS['post']->ID ), 'large' );
							$img = $img_src[0];						
							$thumbnail = aq_resize( $img, $list_small_width, $list_small_height, $list_small_crop, true, true );
						...
						$thumblink = sprintf( '<div class="post-thumb"><a href="%1$s" title="%2$s"><img src="%3$s" alt="%2$s" title="%2$s"/>%4$s</a></div>', $permalink, $title, $thumbnail, $video_icon );
					}
					else {
						$thumblink = '';
						$post_classes .= ' no-image';
					}
					$format = apply_filters( 'widget_recent_posts_output', '<li class="%1$s">%2$s<div class="post-content"><h4><a href="%3$s" title="%4$s">%4$s</a></h4>%5$s</div></li>' );
					$output.= sprintf( $format, $post_classes, $thumblink, $permalink, $title, $post_meta );
                endwhile;
                $output .= '</ul>';
				echo $output;
				echo $after_widget; ?>
